...
# Line 5 >> This line displays a message on the screen, but will only be executed if the number is positive and no error occurs. f-string (formatted string f) is used to insert the value of number**2(squared number) into the message. # Esta linha mostra uma mensagem na tela, mas somente será executada se o número for positivo e nenhum erro ocorrer. f-string (cadeia formatada f) é utilizada para inserir o valor de number**2(numero ao quadrado) na mensagem. 
